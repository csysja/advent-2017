...
const result = passPhrases.map(isValid2)
    .reduce((a, b) => a + b);  
console.log(result);

function isValid2(passPhrase) {
    const words = passPhrase.split(' ');
    var hasDuplicatesOrAnagram = words.some((w, i) => words.some((w2, i2) => i !== i2 && isAnagramOrDuplicate(w, w2)));
    return !hasDuplicatesOrAnagram;
}

function isAnagramOrDuplicate(word1, word2) {
    return word1 === word2 || isAnagram(word1, word2);
}
...
function isAnagram(word1, word2) {
    ...
    while(i < array1.length && index > -1);
    return i === array1.length && array2.length === 0 && index > -1;
}

function isValid(passPhrase) {
    const words = passPhrase.split(' ');
    var hasDuplicates = words.some((w, i) => words.some((w2, i2) => w === w2 && i !== i2));
