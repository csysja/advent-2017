...
function replaceChildPrograms(result, programToMatch) {programToMatch
    result.forEach(program => {
        let i = 0;
        for(let i = 0; i < program.childPrograms.length; i++)
        {
            if (program.childPrograms[i] === programToMatch.programName) {
                program.childPrograms[i] = programToMatch;
                programToMatch.isTopLevel = false;
            }            
        }
    });
}
