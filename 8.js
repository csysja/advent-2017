/*const rawInput = `b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10`;*/

const rawInput = `sd dec 441 if k != 0
lp dec 419 if mxn >= 7
w inc -592 if icg >= -9
a dec -29 if q <= 9
kt dec 486 if ms != 8
...
//const input = getLargestValueInRegister(rawInput); // soltion 1
const input = getLargestValueEverInRegister(rawInput);
console.log(input);

function getLargestValueInRegister(rawInput) {
    let registers = [];
    const input = getInput(rawInput);
    input.forEach(i => {
        let value = getRegisterValue(registers, i.sum.register);        
        if(evaluateCondition(i.condition, registers)) {
            if (i.sum.operator === 'inc') {
                value += i.sum.number; 
            } else {
                value -= i.sum.number;
            }
        }
        setRegisterValue(registers, i.sum.register, value);        
    });
    
    return registers.sort((a, b) => b.value - a.value)[0].value;
}

function getLargestValueEverInRegister(rawInput) {
    let registers = [];
    let totalLargest = 0;
    const input = getInput(rawInput);
    input.forEach(i => {
        ...
        setRegisterValue(registers, i.sum.register, value);        
        const largest = registers.sort((a, b) => b.value - a.value)[0].value;
...
function evaluateCondition(condition, registers) {
    let result;
    const value = getRegisterValue(registers, condition.register);
    switch(condition.operator) {
        case '>':
            result = value > condition.number;
            break;
        case '<':
            result = value < condition.number;
            break;
...
function getRegisterValue(registers, registerName) {
    const register = registers.find(r => r.name === registerName);
    return register === undefined ? 0 : register.value;
}

function setRegisterValue(registers, registerName, registerValue) {
    const register = registers.find(r => r.name === registerName);
    if (register) {
        register.value = registerValue;
    } else {
        registers.push({ name: registerName, value: registerValue });
    }
}
...
function getInput(rawInput) {
    return rawInput.split('\n')
        .map(r => r.split('if'))
        .map(r => ({ sum: r[0].trim().split(' '), condition: r[1].trim().split(' ') }))
        .map(r => ({
            sum: {
                register: r.sum[0],
                operator: r.sum[1],
            ...
            condition: {
                register: r.condition[0],
                operator: r.condition[1],
                number: +r.condition[2]
            }
        }));
}